...
def cnn(X_train):
    # 使用卷积窗口的长度为5，加全局最大池化层，加dropout层（防止过拟合），加softmax分类层

    #model.add(Activation('relu',)
    # model.add(layers.Embedding(1000, 100,input_length = x.shape[1]))笔记本太慢&acc80%左右 不好！
    # model.add(layers.Conv1D(filters=32, kernel_size=3,activation='relu'))
    '''model.add(layers.Conv1D(input_shape=(X_train.shape[1], X_train.shape[2]), filters=64, kernel_size=3, activation='relu'))
    #多一层好像作用不大？
...
if __name__ =='__main__':
    ...
    #图示化训练和验证准确率变化，试验后发现epochs=16比较合适
