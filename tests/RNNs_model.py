...
def rnns(X_train):
    ...
    # model.add(tf.keras.layers.GRU(1, activation='sigmoid', return_sequences=False))

    # 制定激活函数sigmoid，避免RNN、LSTM、GRU报错 'their signatures do not match.' tf.keras.layers.Bidirectional(
    model = tf.keras.Sequential()
